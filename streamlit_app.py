...
def check_password():
    """Verifica credenziali per accesso aziendale privato tramite i Secrets di Streamlit"""
    ...
    if st.button("Accedi"):
        # Controlla la chiave memorizzata nella cassaforte privata dei Secrets
...
# Blocco di sicurezza: l'app si avvia solo se la password è corretta
if check_password():
    st.title("📊 Applicazione Avanzata di Data Visualization")
    st.subheader("Monitoraggio SAL Progetti Minipia")

    # Riquadro sicuro di caricamento file tramite Drag and Drop (Nessun link esposto)
    uploaded_file = st.file_uploader("Trascina qui il file Excel esportato (.xlsx)", type=["xlsx"])

    if uploaded_file is not None:
        try:
            # Lettura dell'intera struttura dei fogli di lavoro
            ...
            # Pulizia delle intestazioni di colonna da spazi spuri e rimozione righe vuote
            ...
            # Elenco delle colonne disponibili nel foglio corrente
            ...
            if len(colonne) >= 2:
                ...
                # Selezione della colonna contenente i nomi dei progetti o delle attività
                col_progetti = st.sidebar.selectbox("Colonna Nomi Progetti:", colonne, index=0)
                
                # Menu per decidere lo stile di visualizzazione grafica preferito
                ...
                # --- CALCOLO PERCENTUALI COMPLESSIVE MACRO AUTOMATICHE ---
                col_fatto_calc = None
                col_da_fare_calc = None
                
                # Individua automaticamente le colonne relative a FATTO e DA FARE
                for col in colonne:
                ...
                # Se entrambe le colonne esistono, esegue la somma matematica e calcola la % complessiva macro
                if col_fatto_calc and col_da_fare_calc:
                    ...
                    if tot_giorni_complessivi > 0:
                        ...
                        # Rendering dei grandi indicatori statistici in cima
                        st.write("### 📈 Stato Avanzamento Complessivo del Foglio (Macro)")
                        macro_col1, macro_col2 = st.columns(2)
                        macro_col1.metric(
                            label="✅ PERCENTUALE COMPLESSIVA FATTA", 
                            value=f"{pct_fatto_complessiva:.1f}%", 
                            delta=f"Somma giorni: {tot_fatto:.1f}"
                        )
                        macro_col2.metric(
                            label="⚠️ PERCENTUALE COMPLESSIVA DA FARE", 
                            value=f"{pct_da_fare_complessiva:.1f}%", 
                            delta=f"Somma giorni: {tot_da_fare:.1f}", 
                            delta_color="inverse"
                        )
                        st.markdown("---")

                # --- MODALITÀ 1: GRAFICO SINGOLO SFUMATO (CLASSICO) ---
                if tipo_grafico == "Singolo Valore (Stile Classico)":
                    ...
                    # Sanificazione dei caratteri percentuali e conversione in formato numerico
                    ...
                    if not df_plot.empty:
                        # Rileva automaticamente se inserire il suffisso % sull'asse dei dati
                        ...
                        fig.update_layout(
                            height=max(400, len(df_plot) * 35), # Layout reattivo per adattarsi al numero di righe
                ...
                # --- MODALITÀ 2: GRAFICO DOPPIO RAGGRUPPATO (AFFIANCATO) ---
                else:
                    default_selezionati = []
                    for col in colonne:
                    ...
                    col_valori = st.sidebar.multiselect(
                        ...
                        colonne, 
                        default=default_selezionati if default_selezionati else [colonne]
                    )
                    
                    if col_valori:
                        st.subheader(f"📈 Grafico di Confronto Interattivo: {selected_sheet}")
                        
                        # Conversione numerica massiva di tutte le colonne selezionate nel multiselect
                        ...
                        if not df_plot.empty:
                            fig = px.bar(
                                ...
                                y=col_progetti, 
                                orientation='h',
barmode='group', # Dispone le colonne raggruppate una di fianco all'altra
color_discrete_sequence=px.colors.qualitative.Pastel,
labels={col_progetti: "Attività / Progetto", "value": "Valore (Giorni / %)", "variable": "Metrica"}
)
fig.update_layout(
...
st.info("💡 Seleziona almeno una colonna nel menu multiselect a sinistra per visualizzare il grafico raggruppato.")
else:
