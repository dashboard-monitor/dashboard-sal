...
def costruisci_attivita(df, nome_foglio):
    ...
    out["Fatto"] = serie_numerica(df.loc[indici, col_fatto]).clip(lower=0) if col_fatto else float("nan")
    out["Da fare"] = serie_numerica(df.loc[indici, col_da_fare]).clip(lower=0) if col_da_fare else float("nan")
