...
def grafico_confronto_team(df, key="grafico_confronto_team_default"):
    righe = []
    ...
    # L'AGGIUNTA FONDAMENTALE È ALLA FINE DI QUESTA RIGA: key=key
...
if vista == "Executive":
    ...
    with col2:
        ...
        # TAB 2: SAL % PER TEAM ORIGINALE (EPAL, MGIO, EPAL+MGIO CONDIVISI)
        with tab_sal:
    ...
    # --- TABELLA 3: PORTAFOGLIO PROGETTI GENERALE ---
    st.markdown("---")
...
# ============================================================
    ...
    if df_db_filt.empty:
        ...
        df_m_p["RANK_MESE"] = df_m_p.groupby("PERIODO")["GIORNI"].rank(method="first", ascending=False)
        df_m_p["COMMESSA_DISPLAY"] = df_m_p.apply(lambda r: r["PROGETTO"] if r["RANK_MESE"] <= 3 else "Altri Progetti", axis=1)

        df_top3_mensile = df_m_p.groupby(["PERIODO", "SORT_KEY", "COMMESSA_DISPLAY"], as_index=False)["GIORNI"].sum().sort_values("SORT_KEY")
        ...
        with col_mon_right:
            df_top_p = df_db_filt.groupby("PROGETTO")["GIORNI"].sum().reset_index().sort_values("GIORNI", ascending=False)
            fig_mon_pie = px.pie(
                df_top_p,
                names="PROGETTO",
                values="GIORNI",
        ...
        st.dataframe(
            ...
            hide_index=True,
            column_config={
                "DATA_COMPLETA": st.column_config.TextColumn("Data", width=110),
                "PROGETTO": st.column_config.TextColumn("Progetto", width=140),
                "TEAM": st.column_config.TextColumn("Team", width=55),
                "ATTIVITÀ": st.column_config.TextColumn("Attività"),
                "MINUTI": st.column_config.NumberColumn("Minuti", format="%d min", width=75),
                "GIORNI": st.column_config.NumberColumn("Giorni", format="%.2f gg", width=75),
            },
        )
